...
import { Search, Clock, Trash2, Copy, Check } from "lucide-react";
...
export function UserResponsesSearch() {
  ...
  const loadRecentResponses = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to load recent responses");
      }
      ...
      <div className="mb-8 w-full max-w-2xl mx-auto">
        {searchType ? (
          <div className="flex justify-center">
            <Button onClick={clearSearch} variant="outline" size="lg" className="px-8">
              New Search
            </Button>
          </div>
        ) : (
            ...
            <h2 className="text-xl font-semibold flex items-center">
              {searchType ? (
                ...
                <>
                  ...
                  Recent Responses ({results.length})
                </>
              )}
            </h2>
          </div>
